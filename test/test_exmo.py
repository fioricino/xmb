...
class TestWorker(unittest.TestCase):
    ...
    def test_get_desired_reserve_price_up(self):
        worker = Worker(None, None, None, stock_fee=0.25)
        need_price = worker._calculate_desired_reserve_price(100, 'UP')
        self.assertEqual(80, need_price)

    def test_get_desired_reserve_amount_up(self):
        worker = Worker(None, None, None, stock_fee=0.2, currency_1_deal_size=4)
        amount = worker._calculate_desired_reserve_amount('UP')
        self.assertEqual(5, amount)

    def test_get_desired_reserve_price_down(self):
        worker = Worker(None, None, None, stock_fee=0.2, currency_1_deal_size=5)
        need_price = worker._calculate_desired_reserve_price(100, 'DOWN')
        self.assertEqual(125, need_price)

    def test_get_desired_reserve_amount_down(self):
        worker = Worker(None, None, None, stock_fee=0.05, currency_1_deal_size=5)
        amount = worker._calculate_desired_reserve_amount('DOWN')
        self.assertEqual(5, amount)

    def test_main_flow_open_profit_order_up_too_many_orders(self):
        # Резервный ордер не создается, если слишком много ордеров на продажу того же профиля
        order_id = '123'
        order = {'order_id': order_id, 'type': 'sell', 'price': str(1100)}
        ...
        class Advisor:
            def get_advice(self):
                return 'UP', 0.002, 1000

        stored_order = {'order_id': order_id, 'status': 'OPEN', 'order_data': order, 'order_type': 'PROFIT',
                        'profile': 'UP'}
        stored_order_s = json.dumps(stored_order)
        storage.orders[order_id] = stored_order
        worker = Worker(api, storage, Advisor(), max_profit_orders_up=1)
        worker.main_flow()

        self.assertEqual(1, api.get_open_orders_called)
        self.assertEqual(stored_order_s, json.dumps(storage.orders[order_id]))

    def test_main_flow_open_profit_order_up_too_small_price_diff(self):
        # Резервный ордер не создается, если слишком большой разброс цен для данного профиля
        order_id = '123'
        order = {'order_id': order_id, 'type': 'sell', 'price': str(1100)}
    ...
    def test_main_flow_open_profit_order_up_too_small_profit(self):
        # Резервный ордер не создается, если слишком маленький профит
        order_id = '123'
        order = {'order_id': order_id, 'type': 'sell', 'price': str(1100)}
    ...
    def test_main_flow_open_profit_order_up_create_reserve(self):
        # Резервный ордер создается
        order_id = '123'
        profit_order = {'order_id': order_id, 'type': 'sell', 'price': str(1100)}
        mock_order_id = '122'
        ...
        stored_order = {'order_id': order_id, 'status': 'OPEN', 'order_data': profit_order, 'order_type': 'PROFIT',
                        'profile': 'UP'}
        stored_order_s = json.dumps(stored_order)
        storage.orders[order_id] = stored_order
        worker = Worker(api, storage, Advisor(), same_profile_order_price_deviation=0.1, profit_markup=0.1,
        ...
        self.assertEquals(profit_order, storage.orders[order_id]['order_data'])
    ...
    def test_main_flow_open_profit_order_down(self):
        ...
        stored_order_s = json.dumps(stored_order)
        storage.orders[order_id] = stored_order
        worker = Worker(api, storage, profile='DOWN')
    ...
    def test_main_flow_order_partially_completed_up(self):
        ...
        api = ApiMock(self)
        worker = Worker(api, storage, profile='UP')
        self.assertRaises(Worker.ScriptQuitCondition, worker.main_flow)
        self.assertEqual(1, api.get_open_orders_called)
    ...
    def test_main_flow_create_reserve_order_up(self):
        ...
        worker = Worker(api, storage, profile='UP', currency_1_deal_size=4, stock_fee=0.2)
    ...
    def test_main_flow_create_reserve_order_down(self):
        ...
        worker = Worker(api, storage, profile='DOWN', currency_1_deal_size=5, stock_fee=0.2)
        worker.get_avg_price = get_avg_price
        worker.main_flow()
    ...
    def test_main_flow_create_profit_order_up(self):
        ...
        worker = Worker(api, storage, profile='UP', stock_fee=0.2,
                        profit_markup=0.2)
        worker.main_flow()
    ...
    def test_main_flow_create_profit_order_down(self):
        ...
        worker = Worker(api, storage, profile='DOWN', stock_fee=0.2,
                        profit_markup=0.2)
        worker.main_flow()
